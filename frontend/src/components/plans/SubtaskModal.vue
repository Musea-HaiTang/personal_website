...
const form = reactive({ name: '', importance: 2, note: '' })

watch(
  () => props.subtask,
  (subtask) => {
    form.name = subtask?.name || ''
    form.importance = subtask?.importance ?? 2
    form.note = subtask?.note || ''
  },
  { immediate: true }
)
...
<template>
  <BaseModal @close="emit('close')">
    <div class="w-full max-w-md rounded-xl bg-card p-6 shadow-xl">
      <h3 class="mb-4 text-lg font-semibold">{{ subtask?.id ? '编辑子任务' : '添加子任务' }}</h3>
      <form @submit.prevent="submit">
        <label class="mb-1 block text-sm text-sub">名字</label>
        <input v-model="form.name" type="text" required class="mb-3 w-full rounded-lg border border-hairline px-3 py-2 text-sm focus:border-teal focus:outline-none" />
