...
const histItems = computed(() => {
  const items = []
  entries.value.forEach((e) => items.push({ kind: 'diary', sort: e.date + 'T00:00:00', label: fmtDate(e.date), title: e.title, tags: e.tags, excerpt: strip(e.content) }))
  flashes.value.forEach((f) => {
    const s = String(f.created_at)
    items.push({ kind: 'flash', sort: s, label: s.slice(5, 10).replace('-', '/') + ' ' + s.slice(11, 16), title: f.content })
  })
    ...
    <div v-if="showHistory" class="overlay" @click.self="showHistory = false">
      <div class="modal">
        ...
        <div class="m-body">
          ...
          <div class="timeline">
            <div v-for="(it, i) in filteredHist" :key="i" class="tl" :class="{ flash: it.kind === 'flash' }">
