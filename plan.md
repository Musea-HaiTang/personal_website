...
- 路由：`/api/tasks` 完整 CRUD；`GET /api/tasks?date=YYYY-MM-DD` 支持按日期筛选；列表按日期、优先级降序排序。
...
- 验证结果：`pytest` 21 项通过；`npm run build` 通过；手工冒烟经前端代理创建会话、当日统计、跨日空统计均正常，测试记录已清理。
...
- 验证结果：`pytest` 18 项通过；`npm run build` 通过；手工冒烟经前端代理完成保存生成 md 文件、编辑、三类搜索命中、删除清理，均正常。
...
- 验证结果：`pytest` 13 项通过；`npm run build` 通过；手工冒烟经前端代理完成新增、列表、日期筛选、完成切换、删除，均正常。
